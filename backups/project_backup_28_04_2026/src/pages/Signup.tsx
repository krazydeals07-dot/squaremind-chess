...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!agreed || !ageConfirmed) {
        setError("Please agree to the Terms, Privacy Policy and confirm you are 13+ years old.");
        return;
    }
    setError('');
    try {
      await signup(email, password, displayName, phoneNumber, dob);
      navigate('/');
    } catch (err) {
    ...
    <Container component="main" maxWidth="sm">
      <Paper elevation={12} sx={{ 
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3, width: '100%' }}>
          ...
          />
            <FormControlLabel
                control={<Checkbox value="age" color="primary" checked={ageConfirmed} onChange={(e) => setAgeConfirmed(e.target.checked)} sx={{color: '#94a3b8'}}/>}
                label={
